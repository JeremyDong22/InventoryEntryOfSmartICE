...
const AppContent: React.FC = () => {
  ...
  const [currentView, setCurrentView] = useState<AppView>(AppView.NEW_ENTRY); // 默认开始录入
  ...
  return (
    <div className="fixed inset-0 flex text-primary font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col h-full relative w-full">
        {/* Mobile Header Button - 仅在非全屏页面显示 */}
        {currentView !== AppView.NEW_ENTRY &&
         currentView !== AppView.PROFILE &&
         currentView !== AppView.APPROVAL && (
        ...
        <main className={`flex-1 ${currentView === AppView.DASHBOARD ? 'overflow-hidden' : 'overflow-y-auto'} ${currentView === AppView.NEW_ENTRY ? 'p-0' : 'p-4 md:p-8'} max-w-5xl mx-auto w-full`}>
            ...
            {currentView === AppView.NEW_ENTRY && <EntryForm onSave={handleSaveEntry} userName={displayName} />}
            {currentView === AppView.HISTORY && <HistoryView />}
            {currentView === AppView.PROFILE && <ProfilePage onNavigate={setCurrentView} />}
            {currentView === AppView.APPROVAL && <ApprovalPage onNavigate={setCurrentView} />}
        </main>
      </div>
    </div>
  );
